  ...
  <div class="rolesControl">
    <el-card>
      <el-button type="primary"
                 @click="addRolesTab">添加权限</el-button>
      <el-table class="mtop30"
                :data="rolesTab"
                stripe
                border
                style="width: 100%;">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary"
                       @click="editRoles(scope.$index, scope.row)">编辑</el-button>
            <el-button type="warning"
                       @click="deleteRoles(scope.$index)"
                       :disabled="isAdmin(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
    <el-dialog title="权限编辑"
      ...
      <el-form ref="rolesForm"
        ...
        <el-form-item>
          <el-button type="primary"
                     @click="changeRoles('rolesForm', editType)">确认</el-button>
...
export default {
  ...
  methods: {
    ...
    //hasOnlyChild遍历出值只有1个节点的菜单，包括所有子节点
    hasOnlyChild(children = [], item) {
      ...
      if (newChildren.length === 1 && !item.meta) {  //只有一个节点的一级菜单
        return newChildren[0]
      } else if (newChildren.length === 0) {  //子节点
        return item
      }
      return false   //多节点根目录返回false
    }
  }
}
</script>
